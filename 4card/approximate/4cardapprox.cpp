...
bool isRummyAceLow(set<int> _cards){
	int i = 0;
	int c[4];
...
int main( int argc, char *argv[] ){
	if ( argc < 2 ) // argc should be 2 for correct execution
			...
			for(int i=h+1; i<50; i++){
				for(int j=i+1; j<51; j++){
					for(int k=j+1; k<52; k++){
						...
						c.insert(k);
						idxs[c] = counter;
						hands[counter++] = new Hand(c, 4, hands, &idxs, isRummyAceLow);
					}
				}
			}
		}
		int locked = 0;
		ofstream outsum; 
		outsum.open("4lowsum100.txt");
		ofstream outfull;
		outfull.open("4lowfull100.txt");
		...
		for(int i=1; i<argc; i++){
			...
			if (*p != '\0' || errno != 0) {
				...
				double tol = 1.0/arg;
				string sarg(argv[i]);
				string sumOutStr = "4lowApprox" + sarg + ".txt";
				string fullOutStr = "4lowApprox" + sarg + "full.txt";
