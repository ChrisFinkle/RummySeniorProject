...
bool isRummyAcesHigh(set<int> _cards){
	int i = 0;
	int c[3];
	...
	if(c[0]%4==c[1]%4 && c[1]%4==c[2]%4){
		if((c[1]/4-c[0]/4==1 && c[2]/4-c[1]/4==1) || (c[0]/4==0 && c[1]/4==11)){
			return true;
		}
	}
	if(c[0]/4==c[1]/4 && c[1]/4==c[2]/4){
...
int main(){
	...
	for(int i=0; i<50; i++){
		for(int j=i+1; j<51; j++){
			for(int k=j+1; k<52; k++){
				...
				c.insert(k);
				idxs[c] = counter;
				hands[counter++] = new Hand(c, 3, hands, &idxs, isRummyAcesHigh);
			}
		}
	}
	...
	ofstream outfull;
	outfull.open("3aceHighResultsFull.txt");
	int locked = 0;
	for(int i=0; i<22100; i++){
	...
	ofstream outsum; 
	outsum.open("3aceHighResults.txt");
	while(locked<22100){
		bool xf = false;
		string xhand = "";
