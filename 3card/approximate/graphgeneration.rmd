...
```{r}
a.4.high <- read.csv("../../4card/approximate/4Approx1000full.txt") %>% arrange(E) %>% mutate(Rule = "Aces High")
a.4.wrap <- read.csv("../../4card/deterministic/4wrapfull.txt", header=F) %>% arrange(V2) %>% mutate(Rule = "Aces Wrap")
a.4.low <- a.4.1000 %>% arrange(E) %>% mutate(Rule = "Aces Low")

a.4.high <-a.4.high %>% filter(E>0)
colnames(a.4.wrap) <- c("Hand", "E", "Rule")
a.4.high$Hand <- 58:270725
a.4.wrap$Hand <- 66:270725
a.4.low$Hand <- 54:270725

data11 <- rbind(rbind(a.4.wrap, a.4.high), a.4.low)
fig12 <- ggplot(data11, aes(x=Hand, y=E, color=Rule)) +
  geom_line() +
  guides(colour = guide_legend(override.aes = list(size=5))) +
  scale_y_continuous("Expected Turns to Rummy") +
  scale_x_continuous("Cumulative Hands Evaluated", labels=comma) +
  scale_color_manual("Rule Set", values = c("dark green","blue","red"))+
  theme(panel.background = element_blank(),
        panel.border = element_rect(color="black",fill=NA,size=1))
fig12
ggsave("../../images/fig12.png", fig12, width = 8, height = 5)
```